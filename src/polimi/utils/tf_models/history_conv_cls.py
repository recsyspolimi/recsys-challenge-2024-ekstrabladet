...
class TemporalConvolutionalHistoryClassificationModel(TabularNNModel):
    ...
    def _build(self):
        ...
        for i in range(self.n_conv_layers):
            x = tfkl.Conv1D(self.conv_filters, 
                            ...
                            name=f'Conv{i}')(x)
            x = tfkl.BatchNormalization(name=f'ConvBatchNormalization{i}')(x)
            x = tfkl.Activation(self.conv_activation, name=f'ConvActivation{i}')(x)
            x = tfkl.Dropout(self.dropout_rate, name=f'ConvDropout{i}')(x)
            dilation *= 2
            
        x = tfkl.Flatten()(tfkl.Cropping1D(cropping=(self.window_size-1, 0))(x))
